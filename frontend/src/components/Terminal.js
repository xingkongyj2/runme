...
const TerminalComponent = ({ hostId, hostIP, onClose, fullscreen = false }) => {
  ...
  useEffect(() => {
    ...
    terminal.current = new Terminal({
      ...
      cols: 120,
      rows: 30,
      convertEol: true, // 自动转换行尾字符
      disableStdin: false,
      cursorStyle: 'block',
      scrollback: 1000, // 增加滚动缓冲区
      allowTransparency: true,
      macOptionIsMeta: true, // Mac 兼容性
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* 弹窗容器 - 保持边框并优化阴影 */}
      <div className="relative w-full max-w-6xl h-[90vh] bg-card rounded-xl border border-border flex flex-col" 
           style={{
             boxShadow: '0 8px 25px -5px rgba(0, 0, 0, 0.15), 0 4px 12px -4px rgba(0, 0, 0, 0.1), inset 0 0 0 1px rgba(255, 255, 255, 0.05)'
           }}>
        {/* 弹窗头部 - 与其他模块保持一致 */}
        <div className="flex items-center justify-between p-6 border-b border-border flex-shrink-0">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-semibold text-foreground">SSH终端</h2>
            <span className="text-sm text-white font-mono bg-blue-600 px-2 py-1 rounded">
              {hostIP || 'IP未获取'}
            </span>
            {/* 连接状态指示器 */}
